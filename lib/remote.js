...
// Open a download stream for a remote file. Resolves symlinks first so the
// caller can re-check the REAL path against its allowlist before streaming.
function openDownload(serverId, remotePath) {
  return getClient(serverId).then((client) => new Promise((resolve, reject) => {
    client.sftp((err, sftp) => {
      if (err) return reject(err);
      const fail = (e) => { try { sftp.end(); } catch (_) {} reject(e); };
      sftp.realpath(remotePath, (err, real) => {
        if (err) return fail(new Error('not found: ' + remotePath));
        sftp.stat(real, (err, st) => {
          if (err) return fail(new Error('not found: ' + real));
          resolve({
            real, size: st.size, isFile: st.isFile(),
            stream: () => { const rs = sftp.createReadStream(real); rs.on('close', () => { try { sftp.end(); } catch (_) {} }); return rs; },
            close: () => { try { sftp.end(); } catch (_) {} },
          });
        });
      });
    });
...
module.exports = { getClient, execOnServer, openRemotePty, streamOnServer, probeServer, uploadBuffer, openDownload, closeAll, shq };
