...
function sanitizeServer(input, existing) {
  ...
  if (!load().keys.some(k => k.id === keyId)) throw new Error('Unknown key');
  return { name, host, port, user, keyId };
}
...
function listServers() {
  return load().servers.map(({ id, name, host, port, user, keyId, createdAt }) => ({ id, name, host, port, user, keyId, createdAt }));
}
...
module.exports = {
  setSecret, addKey, listKeys, getKeyMaterial, removeKey,
  addServer, updateServer, removeServer, listServers, getServer, pinHostKey,
