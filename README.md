...
- The **key bar** sends special keys: Enter, shift-tab, arrows, tab, scroll (screen copy mode — drag or arrows to scroll history, esc to exit), pgup/pgdn, esc, ^C, paste.
- **full** hides all headers and bars so the terminal fills the screen — tap **exit full** (top-right) to bring the chrome back.
